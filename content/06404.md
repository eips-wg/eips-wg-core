...
1. **Transaction inclusion proofs:** Changing the transaction representation to [EIP-6493 `SignedTransaction`](https://eips.fyi/6493) commits to the transaction root hash on-chain, allowing verification of the list of all transaction hashes within a block, and allowing compact transaction inclusion proofs.
...
Note that for SSZ transactions with `tx.signature.type_ == TRANSACTION_TYPE_SSZ`, the `tx_hash` is equivalent to the `tx_root`. Like the `tx_hash`, the `tx_root` remains perpetually [stable](https://eips.fyi/7495) across future upgrades.

It is RECOMMENDED that implementations introduce indices for tracking transactions by `tx_root`.

## Rationale

This change enables the use of SSZ transactions as defined in [EIP-6493](https://eips.fyi/6493).
