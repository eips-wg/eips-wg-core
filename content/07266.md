...
This EIP removes the [`blake2f`](https://eips.fyi/152) (`0x09`) precompile by changing the precompile behaviour to result in an exceptional abort.

## Motivation

[EIP-152](https://eips.fyi/152) has never capitalised on a real-world use case. This fact is clearly reflected in the number of times the address `0x09` has been invoked (numbers from the date this EIP was created):
...
One of the reasons why [EIP-152](https://eips.fyi/152) has failed is that the envisioned use cases were not validated before inclusion.
