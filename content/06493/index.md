...
For existing [EIP-2718](https://eips.fyi/2718) Recursive-Length Prefix (RLP) transactions, these hashes are based on a linear keccak256 hash across their serialization.
...
### [EIP-2718](https://eips.fyi/2718) transaction types

| Name | SSZ equivalent | Description |
| - | - | - |
| `TransactionType` | `uint8` | [EIP-2718](https://eips.fyi/2718) transaction type, range `[0x00, 0x7F]` |

The values `0x00` and `0x04` are marked as reserved [EIP-2718](https://eips.fyi/2718) transaction types.

- `0x00` indicates an [EIP-2718](https://eips.fyi/2718) `LegacyTransaction`.
- `0x04` indicates an SSZ `SignedTransaction` as defined in this EIP.

| Name | Value | Description |
| - | - | - |
| (n/a) | `None` | Untyped [`LegacyTransaction`](https://eips.fyi/2718#transactions) ('Homestead' scheme) |
| `TRANSACTION_TYPE_LEGACY` | `TransactionType(0x00)` | Untyped [`LegacyTransaction`](https://eips.fyi/2718#transactions) ([EIP-155 scheme](https://eips.fyi/155)) |
| `TRANSACTION_TYPE_EIP2930` | `TransactionType(0x01)` | [EIP-2930](https://eips.fyi/2930#definitions) transaction |
| `TRANSACTION_TYPE_EIP1559` | `TransactionType(0x02)` | [EIP-1559](https://eips.fyi/1559#specification) transaction |
| `TRANSACTION_TYPE_EIP4844` | `TransactionType(0x03)` | [EIP-4844](https://eips.fyi/4844#parameters) transaction |
| `TRANSACTION_TYPE_SSZ` | `TransactionType(0x04)` | SSZ `SignedTransaction` |

Note that `0x19` is reserved to prevent collision with [ERC-191](https://eips.fyi/191) signed data.
...
All SSZ transactions are represented as a single, normalized SSZ container. The definition uses the `StableContainer[N]` SSZ type and `Optional[T]` as defined in [EIP-7495](https://eips.fyi/7495).
...
Valid transaction types can be defined using [EIP-7495](https://eips.fyi/7495) `Variant`.
...
Such changes [do not affect](https://eips.fyi/7495) how existing transactions serialize or merkleize.
...
| Name | SSZ equivalent | Description |
| - | - | - |
| `ChainId` | `uint256` | [EIP-155](https://eips.fyi/155) chain ID at time of signature |
...
See [EIP assets](./assets/tx_hashes.py) for a definition of `compute_sig_hash` that takes the various transaction types into account.

### SSZ `PooledTransaction` container

During transaction gossip responses ([`PooledTransactions`](https://github.com/ethereum/devp2p/blob/6b259a7003b4bfb18365ba690f4b00ba8a26393b/caps/eth.md#pooledtransactions-0x0a)), each `SignedTransaction` is wrapped into a `PooledTransaction`. The definition uses the `StableContainer[N]` SSZ type and `Optional[T]` as defined in [EIP-7495](https://eips.fyi/7495).
...
The same additional validation constraints as defined in [EIP-4844](https://eips.fyi/4844) also apply to transactions that define `tx.payload.blob_versioned_hashes` or `blob_data`.
...
Such changes [do not affect](https://eips.fyi/7495) how existing pooled transactions serialize, merkleize, or validate.

### SSZ `Receipt` container

All SSZ receipts are represented as a single, normalized SSZ container. The definition uses the `StableContainer[N]` SSZ type and `Optional[T]` as defined in [EIP-7495](https://eips.fyi/7495).
...
Valid receipt types can be defined using [EIP-7495](https://eips.fyi/7495) `Variant`.
...
Such changes [do not affect](https://eips.fyi/7495) how existing receipts serialize or merkleize.

![Receipt merkleization](./assets/receipt.png)

### Networking

When exchanging SSZ transactions and receipts via the [Ethereum Wire Protocol](https://github.com/ethereum/devp2p/blob/6b259a7003b4bfb18365ba690f4b00ba8a26393b/caps/eth.md), the following [EIP-2718](https://eips.fyi/2718) compatible envelopes are used:
...
The semantics of the [`types` element](https://eips.fyi/5793) in transaction gossip announcements ([`NewPooledTransactionHashes`](https://github.com/ethereum/devp2p/blob/6b259a7003b4bfb18365ba690f4b00ba8a26393b/caps/eth.md#newpooledtransactionhashes-0x08)) is changed to match `ssz(PooledTransaction.active_fields())`:

| `types` | Description |
| - | - |
| `0x00` | Untyped [`LegacyTransaction`](https://eips.fyi/2718#transactions) ('Homestead' scheme, or [EIP-155 scheme](https://eips.fyi/155)) |
| `0x01` | [EIP-2930](https://eips.fyi/2930) transaction, or basic SSZ `PooledTransaction` without any additional auxiliary payloads |
| `0x02` | [EIP-1559](https://eips.fyi/1559) transaction |
| `0x03` | [EIP-4844](https://eips.fyi/4844) transaction, or SSZ `PooledTransaction` with `blob_data` |
...
All SSZ transactions (including future ones) share the single [EIP-2718](https://eips.fyi/2718) transaction type `TRANSACTION_TYPE_SSZ`. Future features can introduce new optional fields as well as new allowed combination of optional fields, as determined by `select_variant` in `AnySignedTransaction`.
...
The `types` element as introduced in eth/68 via [EIP-5793](https://eips.fyi/5793) allows the receiving node better control over the data it fetches from the peer and allows throttling the download of specific types.
