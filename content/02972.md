...
## Abstract
Introduces two new [EIP-2718](https://eips.fyi/2718) transactions that are signature compatible with legacy transactions and can be automatically upgraded by any client.
...
This EIP provides a mechanism for transmitting/including transactions in a way that is [EIP-2718](https://eips.fyi/2718) compatible while still being signature compatible with legacy transactions.
...
Legacy transactions introduced chain ID in [EIP-155](https://eips.fyi/155) and wanted to avoid changing the transaction array length, so it bitpacked the chainID into the signature's `v` value.
...
[EIP-706](https://eips.fyi/706) limits devp2p messages to 24 bit length, which gives us a pragmatic cap at that for any single transaction at the moment.
...
While [EIP-2718](https://eips.fyi/2718) strongly recommends including the transaction type as the first byte of the signed data, we cannot accomplish that in this case because we need to remain signature compatible with legacy transactions.
Luckily, [EIP-2718](https://eips.fyi/2718) also excludes transaction types `0xc0` to `0xfe` from valid transaction types, and the first byte of the signature in this case is in that range so we can be sure this will not conflict with any future transaction types.
