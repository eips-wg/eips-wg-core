...
2. Replace `SSTORE_CLEARS_SCHEDULE` (as defined in [EIP-2200](https://eips.fyi/2200)) with `SSTORE_RESET_GAS + ACCESS_LIST_STORAGE_KEY_COST` (4,800 gas as of [EIP-2929](https://eips.fyi/2929) + [EIP-2930](https://eips.fyi/2930))
...
In [EIP-2200](https://eips.fyi/2200#specification), three cases for refunds were introduced:
...
Note, there is a difference between 'hot' and 'cold' slots. This table shows the values as of [EIP-2929](https://eips.fyi/2929) assuming that all touched storage slots were already 'hot' (the difference being a one-time cost of `2100` gas).
