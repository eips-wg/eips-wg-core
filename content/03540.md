...
We introduce an extensible and versioned container format for the EVM with a once-off validation at deploy time. The version described here brings the tangible benefit of code and data separation, and allows for easy introduction of a variety of changes in the future. This change relies on the reserved byte introduced by [EIP-3541](https://eips.fyi/3541).
...
EOF version 1 is made up of 5 EIPs, including this one: [EIP-3540](https://eips.fyi/3540), [EIP-3670](https://eips.fyi/3670), [EIP-4200](https://eips.fyi/4200), [EIP-4750](https://eips.fyi/4750), and [EIP-5450](https://eips.fyi/5450). Some values in this specification are only discussed briefly. To understand the full scope of EOF, it is necessary to review each EIP in-depth.
...
See [EIP-4750](https://eips.fyi/4750) for more information on the type section content.
...
6. The size limit for deployed code as specified in [EIP-170](https://eips.fyi/170) and for initcode as specified in [EIP-3860](https://eips.fyi/3860) is applied to the entire *container* size, not to the *code* size. This also means if initcode validation fails, it is still charged the EIP-3860 `initcode_cost`.
...
(*Remark:* Due to [EIP-4750](https://eips.fyi/4750), `JUMP` and `JUMPI` are disabled and therefore are not discussed in relation to EOF.)
...
- The biggest disadvantage is that deploy-time validation of EOF code must be enabled in two hard-forks. However, the first step ([EIP-3541](https://eips.fyi/3541)) is already deployed in London.
...
1. The first byte `0xEF` was chosen because it is reserved for this purpose by [EIP-3541](https://eips.fyi/3541).
...
Currently contracts can selfdestruct in three different ways (directly through `SELFDESTRUCT`, indirectly through `CALLCODE` and indirectly through `DELEGATECALL`). [EIP-3670](https://eips.fyi/3670) disables the first two possibilities, however the third possibility remains. Allowing EOF1 contracts to only `DELEGATECALL` other EOF1 contracts allows the following strong statement: EOF1 contract can never be destructed. Attacks based on `SELFDESTRUCT` completely disappear for EOF1 contracts. These include destructed library contracts (e.g. Parity Multisig).
...
- [EIP-3860](https://eips.fyi/3860) for *initcode*,
