...
This EIP allows for tail call optimizations in EOF functions ([EIP-4750](https://eips.fyi/4750)) by introducing a new instruction `JUMPF`, which jumps to a code section without adding a new return stack frame.
...
Let the definition of `type[i]` be inherited from [EIP-4750](https://eips.fyi/4750) and define `stack_height` to be the height of the stack at a certain instruction during the instruction flow traversal if the operand stack at the start of the function were equal to `type[i].inputs`.
...
* The code validation defined in [EIP-4200](https://eips.fyi/4200) also fails if any `RJUMP*` offset points to one of the two bytes directly following a `JUMPF` instruction.
