...
This EIP specifies the inclusion of the `forkid`, originally defined in [(EIP-2124)](https://eips.fyi/2124), as a new field in the Ethereum wire protocol (`eth`) handshake. This change is implemented as a new version of the wire protocol, `eth/64`.

## Motivation

The [`forkid` (EIP-2124)](https://eips.fyi/2124) was designed to permit two Ethereum nodes to quickly and cheaply decide if they are compatible or not, not only at a genesis/networking level, but also from the perspective of the currently passed network updates (i.e. forks).

[EIP-2124](https://eips.fyi/2124) only defines how the `forkid` is calculated and validated, but does not specify how the `forkid` should be exchanged between peers. This EIP specifies the inclusion of the `forkid` as a new field in the Ethereum wire protocol (`eth`) handshake (releasing a new version, `eth/64`).
...
- Implement `forkid` generation and validation per [EIP-2124](https://eips.fyi/2124).
...
- Redefine `Status (0x00)` for `eth/64` to add a trailing `forkid` field:
  ...
  where `forkid` is `[forkHash: [4]byte, forkNext: uint64]` (fields per [EIP-2124](https://eips.fyi/2124) ).
...
For calculating and validating fork IDs, see test cases in [EIP-2124](https://eips.fyi/2124).
